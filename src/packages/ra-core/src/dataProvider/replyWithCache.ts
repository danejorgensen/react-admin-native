...
export const canReplyWithCache = (type: string, payload: any, resourceState: any) => {
    const now = new Date();
    switch (type) {
        case 'getList':
            return (
...
export const getResultFromCache = (type: string, payload: any, resourceState: any) => {
    switch (type) {
        case 'getList': {
        ...
        case 'getMany':
            return {
                data: payload.ids.map((id: any) => resourceState.data[id]),
            } as GetManyResult;
        default:
