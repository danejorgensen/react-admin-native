...
const makeGetManySelector = () =>
    createSelector(
        [
            (state: ReduxState, resource: any) =>
                get(state, ['admin', 'resources', resource, 'data']),
            (_, __, ids) => ids,
        ],
        (resourceData: any, ids: any[]) =>
            resourceData
                ? ids.map((id: any) => resourceData[id])
                // @ts-ignore
                : ids.map((id: any) => undefined)
    );
...
const callQueries = debounce(() => {
    ...
    resources.forEach((resource: any) => {
        ...
        const accumulatedIds = queries
            // @ts-ignore
            .reduce((acc: any, { ids }) => union(acc, ids), []) // concat + unique
                    ...
                    queries.forEach(({ ids, setState, onSuccess }: any) => {
                        ...
                        if (onSuccess) {
                            const subData = ids.map(
                                (id: any) =>
                                    response.data.find((datum: any) => datum.id == id) // eslint-disable-line eqeqeq
                            );
